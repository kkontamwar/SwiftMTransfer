...
namespace SwiftMTransfer.BusinessLayer
{
	public static class TSQL
	{
		...
		public static string Build_Add_Money(AddMoneyDetail value, string Credit, string Debit, string Balance)
		{
			return string.Format(@"INSERT INTO UserTransaction (AccountNumber, TransactionDate,Credit,Debit,Balance,TransactionStatus,TransactionDescription)
					  VALUES   ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}');SELECT SCOPE_IDENTITY();", value.AddMoney.ToUser.AccountNumber, DateTime.Now, Credit, Debit, Balance, "Sucess", value.AddMoney.Description);
		}
